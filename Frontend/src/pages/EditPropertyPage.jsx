  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        ...
        if (!cancelled) {
          const p = data.data;
          setForm({ title: p.title, description: p.description, price: p.price, city: p.city, country: p.country, type: p.type, listingType: p.listingType, imageUrls: p.imageUrls?.join(', ') || '' });
        }
      } catch (err) {
        if (!cancelled) setServerError(err.response?.data?.message || 'Failed to load');
      ...
      <div className="form-card">
        ...
        <form className="property-form" onSubmit={handleSubmit} noValidate>
          <div className="form-row">
            <InputField id="title" label="Property Title" value={form.title} onChange={handleChange} error={errors.title} required />
            <InputField id="price" label="Price (USD)" type="number" value={form.price} onChange={handleChange} error={errors.price} required min="0" />
          </div>
          <InputField id="description" label="Description" error={errors.description} required>
            <textarea id="description" className={`input-field textarea ${errors.description ? 'input-error' : ''}`} value={form.description} onChange={handleChange} rows={4} />
          </InputField>
          <div className="form-row">
          ...
          <InputField id="imageUrls" label="Image URLs (comma-separated)" value={form.imageUrls} onChange={handleChange} placeholder="https://..." />
