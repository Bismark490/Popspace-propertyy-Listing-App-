...
const CreatePropertyPage = () => {
  ...
  const [form, setForm] = useState({ title: '', description: '', price: '', city: '', country: '', type: '', listingType: 'rent', imageUrls: '' });
      ...
      <div className="form-card">
        ...
        <form className="property-form" onSubmit={handleSubmit} noValidate>
          <div className="form-row">
            <InputField id="title" label="Property Title" value={form.title} onChange={handleChange} placeholder="e.g. Cozy 2-Bedroom Apartment" error={errors.title} required />
            <InputField id="price" label="Price (USD)" type="number" value={form.price} onChange={handleChange} placeholder="e.g. 1200" error={errors.price} required min="0" />
          </div>
          <InputField id="description" label="Description" error={errors.description} required>
            <textarea id="description" className={`input-field textarea ${errors.description ? 'input-error' : ''}`} value={form.description} onChange={handleChange} placeholder="Describe the property..." rows={4} />
          </InputField>
          <div className="form-row">
            <InputField id="city" label="City" value={form.city} onChange={handleChange} placeholder="e.g. London" error={errors.city} required />
            <InputField id="country" label="Country" value={form.country} onChange={handleChange} placeholder="e.g. United Kingdom" error={errors.country} required />
          </div>
          <div className="form-row">
            <InputField id="type" label="Property Type" error={errors.type} required>
              <select id="type" className={`input-field filter-select ${errors.type ? 'input-error' : ''}`} value={form.type} onChange={handleChange}>
          ...
          <InputField id="imageUrls" label="Image URLs (comma-separated, optional)" value={form.imageUrls} onChange={handleChange} placeholder="https://example.com/img1.jpg, ..." />
